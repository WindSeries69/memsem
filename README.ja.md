...
- 🔄 **自己修正する。** 矛盾は古い事実を上書きするのではなく、薄れさせていく（「何年も牛乳を飲んできた…待って、乳糖不耐症だ」）— 履歴は常に保持され、重要な事実（≥ 0.9）は保護される。バックグラウンドエージェントがセッション終了時に持続的な事実を抽出し、小さな事実をパターンに統合し、優先度を再調整する — それはメモリが*少なくとも同じくらい検索しやすい*状態に留まる場合に限られる。
