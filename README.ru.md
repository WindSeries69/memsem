...
- 🔄 **Самокорректируется.** Противоречия затухают старый факт вместо того, чтобы
  ...
  история всегда сохраняется, критически важные факты (≥ 0.8) защищены. Фоновые
...
Читайте честно — из независимого обзора ([Agent Memory Atlas](https://neoneye.github.io/agent-memory-atlas/systems/memsem/)):

- **На автоматическом пути исправлений нет блокировки.** Отклонённое значение,
  *утверждённое заново* (например, один и тот же старый транскрипт прочитан десять
  раз), возвращается и затухает собственную коррекцию — обычная коррекция
  архивируется при третьем повторном утверждении. Только **человек, отклоняющий
  кандидата**, записывает устойчивое подавление (`memory_suppressions`), которое
  отбрасывает значение напрямую. Это осознанная позиция (повторение — это
  доказательство) с реальной ценой.
- **Фиксация защищает выживание, а не видимость.** Закреплённая коррекция никогда
  не теряет уверенность и остаётся первой в `memsem list`, но повторно отклонённое
  значение всё ещё может занять верхнюю позицию в `memory_search`.
- **`import` пишет мимо ворот** — восстановление резервной копии возвращает
  подавленное значение.
- **Отказанная запись не оставляет строки аудита**, а очистка проверенного факта
  оставляет его текст в `memory_candidates`.
- **Правила безопасности консолидации и извлечения — это промпты, а не код.**

Острые углы, а не баги — каждый отслеживается в [DESIGN.md](DESIGN.md) дорожной
карте и открытых вопросах.
...
- [x] Распространение по графу с несколькими переходами
- [ ] Write gate на автоматическом пути (решение supersession → suppression)
- [ ] `import` за воротами (консультироваться с suppression)
- [ ] Аудит отказов ; очистка кандидатов ; правила консолидации в коде
